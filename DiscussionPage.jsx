...
const CATEGORIES = [
  ...
  { value: "competitive-programming", label: "CP",         icon: "🏆" },
  { value: "interview-experience",    label: "Interviews", icon: "🎯" },
  { value: "placement-prep",          label: "Placements", icon: "💼" },
  { value: "tech-discussion",         label: "Tech",       icon: "⚡" },
  { value: "career-advice",           label: "Career",     icon: "🚀" }
];
...
function DiscussionPage() {
  ...
  useEffect(() => {
    fetchDiscussions(search, activeCategory);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [activeCategory]);

  const navigate=useNavigate();
  const fetchDiscussions = async (searchTerm = "", categoryArg = activeCategory) => {
    try {
      setError(null);
  ...
  const handleSearchChange = (e) => {
    ...
    debounceTimeoutRef.current = setTimeout(() => {
      fetchDiscussions(value, activeCategory);
    }, 300);
  };
            ...
            {discussions.length > 0 ? (
              discussions.map((discussion) => (
                <DiscussionCard 
                  ...
                  onClick={()=> handleclick(discussion._id)/*() => openDiscussionDetails(discussion)*/ }
                />
              ))
            ) : (
