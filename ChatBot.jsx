...
const ChatBot = () => {
    ...
    const handleSendMessage = async (e) => {
        ...
        try {
            ...
            const botRagUsed = !!response.data.ragUsed;
            const botSourcesCount = response.data.sourcesCount || 0;
            setRagUsed(botRagUsed);
            setSourcesCount(botSourcesCount);
            setMessages(prev => [...prev, {
                text: response.data.response,
                sender: 'bot',
                ragUsed: botRagUsed,
                sourcesCount: botSourcesCount
            }]);
            ...
            <div className="flex-1 overflow-y-auto p-10 space-y-8">
                ...
                {messages.map((message, index) => (
                    ...
                    >
                        ...
                        <div className={`flex flex-col max-w-[95%] ${message.sender === 'user' ? 'items-end' : 'items-start'}`}>
                            ...
                            {message.sender === 'bot' && message.ragUsed && message.sourcesCount > 0 && (
                                <p className="text-xs text-green-500 mt-1 ml-2">
                                    ✓ Based on {message.sourcesCount} campus discussion{message.sourcesCount === 1 ? '' : 's'}
                                </p>
                            )}
                        </div>
                    </div>
